# Order Workflow - NMDP Donor Patient Implementation Guide v0.1.0
...
When a transplant center identifies a potential donor match, it initiates a series of orders through NMDP to confirm compatibility and ultimately collect stem cells. Each order is represented as a [ServiceRequest](http://hl7.org/fhir/R4/servicerequest.html) resource conforming to the [`NMDPDonorOrder`](StructureDefinition-nmdp-donor-order.md) profile.

## Order Types

The following order types are defined in the [`NMDPOrderTypeVS`](ValueSet-nmdp-order-type-vs.md) value set:
...
1. **Infectious Disease Markers (IDM)**— Once typing is confirmed, IDM testing screens the donor for transmissible infections (HIV, Hepatitis B/C, CMV, etc.).
1. **Collection (PBSC or Marrow)**— After medical clearance, the transplant center requests the final stem cell collection, specifying the product type (peripheral blood via apheresis or bone marrow harvest).
...
[`NMDPDonorOrder`](StructureDefinition-nmdp-donor-order.md) is the base profile for all donor orders. It constrains `ServiceRequest` with:
...
[`NMDPIDMOrder`](StructureDefinition-nmdp-idm-order.md) is a child profile that extends `NMDPDonorOrder` specifically for Infectious Disease Marker testing. It adds:
...
* [`ExampleDonorOrder`](ServiceRequest-ExampleDonorOrder.md) — A confirmatory typing order
* [`ExampleIDMOrder`](ServiceRequest-ExampleIDMOrder.md) — An IDM testing order
